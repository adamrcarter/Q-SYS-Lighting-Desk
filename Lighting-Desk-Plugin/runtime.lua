...
NUM_CHANNELS = props["Channels"] -- CHANGE THIS VARIABLE FOR NUMBER OF CHANNELS IN YOUR DESK DESIGN
NUM_SCENES  = props["Scenes"] -- CHANGE THIS VARIABLE FOR NUMBER OF SCENE FADERS IN YOUR DESK DESIGN
...
output.new = function(channel, id, master)
  ...
  self.ouputValue = Controls[string.format("Out %i",id)].Value
  local master = master
...
function handleCHFaderChange(event)
  ...
  Controls[string.format("Out %i", event.Index)].Value = outputArray[event.Index].calculateOutput(subgroupLevel, channelArray, event)
  Controls[string.format("Out %i", event.Index)].String = Controls.Outputs[string.format("Out %i", event.Index)].Value
  if Controls[string.format("Out %i", event.Index)].Value > 0 then Controls.ChannelLED[event.Index].Value = true else Controls.ChannelLED[event.Index].Value = false end
end
...
function handleSceneFaderChange(event)
  ...
  for i=1,NUM_CHANNELS do -- iterate over each ouput and calculate the output value
    ...
    Controls[string.format("Out %i", event.Index)].Value = outputArray[i].calculateOutput(subgroupLevel, channelArray, e) 
    Controls[string.format("Out %i", event.Index)].String = Controls[string.format("Out %i", event.Index)].Valu
    if Controls[string.format("Out %i", event.Index)].Value > 0 then Controls.ChannelLED[event.Index].Value = true else Controls.ChannelLED[event.Index].Value = false end
  end
end
...
function handleMasterFaderChange(event)
  ...
  for i=1,NUM_CHANNELS do -- iterate over each ouput and calculate the output value
    ...
    Controls[string.format("Out %i", event.Index)].Value = outputArray[i].calculateOutput(subgroupLevel, channelArray, ev) 
    Controls[string.format("Out %i", event.Index)].String =  Controls[string.format("Out %i", event.Index)].Value
    if Controls[string.format("Out %i", event.Index)].Value > 0 then Controls.ChannelLED[event.Index].Value = true else Controls.ChannelLED[event.Index].Value = false end
  end
end
